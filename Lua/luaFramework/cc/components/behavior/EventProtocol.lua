...
function EventProtocol:ctor()
    ...
    self.debug_ = true
    -- CCLOGF("I",self.listeners_,self.listenerHandleIndex_,self.debug_)
end
...
function EventProtocol:addEventListener(eventName, listener, data)
    ...
    if self.debug_ then
        if data then
            CCLOGF("EventProtocol:addEventListener() - add listener [%s] %s:%s for event %s", handle, tostring(data), tostring(listener), eventName)
        else
            CCLOGF("EventProtocol:addEventListener() - add listener [%s] %s for event %s", handle, tostring(listener), eventName)
        end
    end
...
function EventProtocol:dispatchEvent(event)
    ...
    if self.debug_ then
        CCLOGF("EventProtocol:dispatchEvent() - dispatching event %s", eventName)
    end
    ...
    for handle, listener in pairs(self.listeners_[eventName]) do
        CCLOGF("FKFKFKF addClearedIndexToRemoveList is loop")
        if self.debug_ then
            CCLOGF("EventProtocol:dispatchEvent() - dispatching event %s to listener [%s]", eventName, handle)
        end
        local ret
        if listener[2] then
        ...
        if ret == false then
            if self.debug_ then
                CCLOGF("EventProtocol:dispatchEvent() - break dispatching for event %s", eventName)
            end
            break
        end
    end
...
function EventProtocol:removeEventListener(eventName, key1, key2)
    ...
    for handle, listener in pairs(self.listeners_[eventName]) do
        if key1 == handle or (key1 == listener[1] and key2 == listener[2]) then
            ...
            if self.debug_ then
                CCLOGF("EventProtocol:removeEventListener() - remove listener [%s] for event %s", handle, eventName)
            end
            return handle
        end
    end
...
function EventProtocol:removeAllEventListenersForEvent(eventName)
    ...
    if self.debug_ then
        CCLOGF("EventProtocol:removeAllEventListenersForEvent() - remove all listeners for event %s", eventName)
    end
    return self
end
...
function EventProtocol:removeAllEventListeners()
    self.listeners_ = {}
    if self.debug_ then
        CCLOGF("EventProtocol:removeAllEventListeners() - remove all listeners")
    end
    return self
end
